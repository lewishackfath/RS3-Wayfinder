...
<div class="page-title-row">
    <div>
        ...
        <p class="muted small">Active profile: <?= e($active['rsn']) ?></p>
    </div>
...
<div class="card">
    <div class="journey-progress-summary">
        <div>
            ...
            <p class="muted"><?= (int)$progress['completed'] ?> of <?= (int)$progress['total'] ?> steps complete</p>
        </div>
        <div class="progress-bar large"><span style="width: <?= e((string)$progress['percent']) ?>%"></span></div>
    </div>
    <?php if (!$playerJourney): ?>
        <form method="post">
            <?= csrf_field() ?>
            <input type="hidden" name="action" value="start">
            <button class="button" type="submit">Start this journey</button>
        </form>
    <?php endif; ?>
</div>
...
<?php foreach ($chapters as $chapter): ?>
    <div class="card journey-chapter">
        ...
        <?php foreach (($stepsByChapter[(int)$chapter['id']] ?? []) as $step): ?>
            <div class="journey-step <?= $step['is_completed'] ? 'complete' : '' ?>">
                <div class="step-status"><?= $step['is_completed'] ? '✓' : '○' ?></div>
                <div class="step-body">
                    <h3><?= e($step['title']) ?></h3>
                    ...
                    <p class="muted small">
                        ...
                        <?php if ($step['is_completed']): ?> • <?= e($step['auto_complete'] ? 'Completed automatically' : 'Completed manually') ?><?php endif; ?>
                    </p>
                </div>
                <div class="step-actions">
                    <?php if ($step['can_complete_manually']): ?>
                        <form method="post">
                            <?= csrf_field() ?>
                            <input type="hidden" name="action" value="toggle_step">
                            <input type="hidden" name="step_id" value="<?= (int)$step['id'] ?>">
                            <input type="hidden" name="completed" value="<?= $step['is_completed'] ? '0' : '1' ?>">
                            <button class="button secondary" type="submit"><?= $step['is_completed'] ? 'Untick' : 'Tick off' ?></button>
                        </form>
                    <?php else: ?>
